...
jQuery( document ).ready( function( $ ){
	...
	$('#posts').change(function(){
		...
		console.log("value in js "+inputValue);
		$.post('/wp-content/themes/vmware-blogs/parts/query-posts.php', { posts: inputValue }, function(data){
			console.log(data)
		});
	});
	...
	if (thehours >= 0 && thehours < 12) {
		$('.morning').removeClass('hide');
	} else if (thehours >= 12 && thehours < 17) {
		$('.afternoon').removeClass('hide');
	} else if (thehours >= 17 && thehours < 24) {
		$('.evening').removeClass('hide');
	}
